...
def main():
    ...
    if any(ch in args.glyph for ch in deny_emoji):
        print(json.dumps({"deny": "emoji"}), file=sys.stderr)
        sys.exit(3)
    if len(acts) > 12:
        print(json.dumps({"deny": "too_many_steps"}), file=sys.stderr)
        sys.exit(3)
    for a in acts:
        if a not in actions:
            print(json.dumps({"error": "unknown_action", "action": a}), file=sys.stderr)
            sys.exit(2)
