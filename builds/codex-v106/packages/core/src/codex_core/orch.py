...
class DAG:
    ...
    def topo(self) -> List[str]:
        """Return topologically sorted list of task names."""
        ...
        g = {}
        for e in self.edges:
            g.setdefault(e.frm, []).append(e.to)
        while q:
            n = q.pop(0)
            out.append(n)
            for m in g.get(n, []):
    ...
    def digest(self) -> str:
        """Compute SHA256 digest of the DAG structure."""
        blob = json.dumps({
            "t": {k: v.__dict__ for k, v in self.tasks.items()},
...
class StepReceipt:
    ...
    def digest(self) -> str:
        """Compute SHA256 digest of the receipt."""
...
class Run:
    ...
    def head(self) -> str:
        """Compute chain head by hashing receipts sequentially."""
        prev = "0" * 64
